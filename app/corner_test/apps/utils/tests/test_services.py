...
from corner_test.apps.utils.exceptions import SlackUserListException,\
    ...
    SlaskUserInfoException, SlackErrorException, SlackSendMessageException
...
class TestSlackService(TestCase):
    ...
    def test_slack_save_employees_error(self):
        with self.assertRaises(SlackUserListException):
            self.service.save_employees(
                mocked_slack_error_user_list()['members'])
    ...
    @mock.patch('slack.WebClient.users_list',
                side_effect=mocked_slack_user_list)
    ...
    @mock.patch('slack.WebClient.conversations_list',
                side_effect=mocked_slack_channel)
    @mock.patch('slack.WebClient.conversations_history',
                side_effect=mocked_slack_messages)
    ...
    @mock.patch('slack.WebClient.chat_postMessage', side_effect=mocked_slack_post_message)
    ...
    @mock.patch('slack.WebClient.chat_postMessage',
                side_effect=mocked_slack_post_message)
    def test_slack_send_messages_with_invalid_auth_2_invalid_user(self, mock_slack_message):
