...
from corner_test.apps.utils.exceptions import SlackUserListException,\
    SlackChannelListException, EmployeeDoesNotExistException,\
    SlaskUserInfoException, SlackErrorException, SlackGetMessageException,\
    SlackGetChannelException, SlackSendMessageException
...
class SlackService():
        ...
        for user in users_array:
            try:
                if user['is_bot'] == False and user['name'] != "Nora":
                    ...
                    Employee.objects.get_or_create(slack_user_id=user_id,
                                                   name=username)
    ...
    def get_users_info(self):
        try:
            ...
            return [user['id'] for user in result["members"] if
                    user['is_bot'] == False and user['name'] != "Nora"]
    ...
    def send_messages(self, users, menu_data):
        ...
        for menu in menu_data['options']:
            data.append({"type": "section",
                         "text": {"type": "plain_text", "text": menu}})
        data.append({"type": "section", "text": {"type": "plain_text",
    ...
    def get_messages(self):
        for user in Employee.objects.all().exclude(name='Slackbot').exclude(name='Nora'):
            ...
            if not channel:
                return
            now = datetime.now()
            init_daily_timestamp = datetime(year=now.year, month=now.month,
                                              day=now.day, hour=8, minute=0,
                                              tzinfo=pytz.timezone(
                                                  'America/Santiago')).timestamp()
            end_limit_timestamp = datetime(year=now.year, month=now.month,
                                              day=now.day, hour=11, minute=0,
                                              tzinfo=pytz.timezone(
                                                  'America/Santiago')).timestamp()
            try:
                results = self.client.conversations_history(channel=channel,
                ...
                if option:
                    ...
                    order, created = Order.objects.get_or_create(dish=dish,
                                         employee=user)
